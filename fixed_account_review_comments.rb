require 'spec_helper'
RSpec.feature 'App platform CD-251620: lookupvalues and account restrcition while creating documents', type: :feature, js: true, accounts: true, account_security: true do
  include SessionHelper
  include SharedHelper

  context 'data creation' do
    let(:req_edit) { RequisitionsPage::Edit.new }
    let(:req_cart) { RequisitionsPage::Cart.new }
    ...
    given!(:us_no_restriction_user) { FactoryGirl.create(:user, :with_roles, roles: "User, Buyer, Accounts Payable, Expense User, Expense Processor", login: "us_no_restriction_user", business_group_security_type: 0, account_security_type: 0) }

    # Users restricted to default chart of account
    given!(:fiji_default_chennai_coa_user) { FactoryGirl.create(:user, :with_roles, roles: "User, Buyer, Accounts Payable, Expense User , Expense Processor", login: "fiji_default_chennai_coa_user", business_group_security_type: 0, account_security_type: 1, default_account_type_id: chennai_coa.id, default_account_id: chennai_coa_100_100_100) }
    given!(:colombia_default_delhi_coa_user) { FactoryGirl.create(:user, :with_roles, roles: "User, Buyer, Accounts Payable, Expense User , Expense Processor", login: "colombia_default_delhi_coa_user", business_group_security_type: 0, account_security_type: 1, default_account_type_id: delhi_coa.id, default_account_id: delhi_coa_200_1982_201_1982_200) }
    given!(:india_default_qatar_coa_user) { FactoryGirl.create(:user, :with_roles, roles: "User, Buyer, Accounts Payable, Expense User , Expense Processor", login: "india_default_qatar_coa_user", business_group_security_type: 0, account_security_type: 1, default_account_type_id: qatar_coa.id, default_account_id: qatar_coa_700_800_700_100_100) }
    given!(:japan_default_doha_coa_user) { FactoryGirl.create(:user, :with_roles, roles: "User, Buyer, Accounts Payable, Expense User , Expense Processor", login: "japan_default_doha_coa_user", business_group_security_type: 0, account_security_type: 1, default_account_type_id: doha_coa.id, default_account_id: doha_coa_8000_1945_8001_1946_900) }

    # User restricted to specific account groups
    given!(:china_user_restrict_to_chennai_coa_ag_by_seg) { FactoryGirl.create(:user, :with_roles, roles: "User, Buyer, Accounts Payable, Expense User , Expense Processor", login: "china_user_restrict_to_chennai_coa_ag_by_seg", business_group_security_type: 0, account_security_type: 2, account_groups: [chennai_coa_ag_by_seg]) }
    given!(:egypt_user_restrict_to_delhi_coa_ag_by_seg) { FactoryGirl.create(:user, :with_roles, roles: "User, Buyer, Accounts Payable, Expense User , Expense Processor", login: "egypt_user_restrict_to_delhi_coa_ag_by_seg", business_group_security_type: 0, account_security_type: 2, account_groups: [delhi_coa_ag_by_seg]) }
    given!(:paris_user_restrict_to_qatar_coa_ag_by_seg) { FactoryGirl.create(:user, :with_roles, roles: "User, Buyer, Accounts Payable, Expense User , Expense Processor", login: "paris_user_restrict_to_qatar_coa_ag_by_seg", business_group_security_type: 0, account_security_type: 2, account_groups: [qatar_coa_ag_by_seg]) }
    given!(:italy_user_restrict_to_doha_coa_ag_by_seg) { FactoryGirl.create(:user, :with_roles, roles: "User, Buyer, Accounts Payable, Expense User , Expense Processor", login: "italy_user_restrict_to_doha_coa_ag_by_seg", business_group_security_type: 0, account_security_type: 2, account_groups: [doha_coa_ag_by_seg]) }
    ...
    shared_examples 'lookup values & account display for chennai_coa' do |user|
      scenario "for #{user}" do
        login_as user
        create_req_line
        req_cart.choose_acc.click
        ##### Lookup value display####
        select_coa chennai_coa.name if user == us_no_restriction_user.login
        wait_for_ajax
        select_coa chennai_coa.name if user == "austria_user_restrict_to_coa_only_AGs"
        wait_for_ajax
        # For Chennai COA By default the third segment should be defaulted from commodity
        expect(req_cart.choose_an_account.segment[2]).to have_content "Red 200 (200)"
        # The frist 4 lookup values from the Red lookup should be displayed
        req_cart.choose_an_account.segment[0].click
        expect(req_cart.segment_1_results.text).to eq "Red 100 (100) Red 200 (200) Red 300 (300) Red 400 (400)"
        req_cart.choose_an_account.segment[0].click
        req_cart.choose_an_account.segment[1].click
        expect(req_cart.segment_2_results.text).to eq "Red 100 (100) Red 200 (200) Red 300 (300) Red 400 (400)"
        req_cart.choose_an_account.segment[1].click
        req_cart.choose_an_account.segment[2].click
        expect(req_cart.segment_3_results.text).to eq "Red 100 (100) Red 200 (200) Red 300 (300) Red 400 (400)"
      end
    end

    shared_examples 'AG validation for chennai_coa' do |user|
      scenario "AG validation for chennai coa with #{user}" do
        login_as user
        create_req_line
        req_cart.choose_acc.click
        req_cart.choose_an_account.segment[0].click
        req_cart.choose_segment[0].search_result[1].click
        req_cart.choose_an_account.segment[1].click
        req_cart.choose_segment[1].search_result[1].click
        req_cart.choose_an_account.choose.click
        expect(req_cart.choose_an_account.errors[0]).to have_content(I18n.t('accounts_controller.your_security_settings_prevent_using_this_account'))
        req_cart.choose_an_account.segment[1].click
        req_cart.choose_segment[1].search_result[2].click
        req_cart.choose_an_account.choose.click
        expect(req_cart.req_cart_line.account_code).to have_content "200-300-200"
        req_cart.submit_for_approval_link.click
      end
    end

    shared_examples 'lookup values & account display for pune_coa' do |user|
      scenario "pune coa for #{user}" do
        login_as user
        create_req_line
        req_cart.choose_acc.click
        ##### Lookup value display#####
        ...
        select_coa pune_coa.name
        wait_for_ajax
        req_cart.choose_an_account.segment[0].click
        expect(req_cart.segment_1_results.text).to eq "Red 100 (100) Red 200 (200)"
        req_cart.choose_an_account.segment[0].click
        req_cart.choose_an_account.segment[1].click
        expect(req_cart.segment_2_results.text).to eq "Red 100 (100) Red 200 (200)"
        req_cart.choose_an_account.segment[1].click
        req_cart.choose_an_account.segment[2].click
        expect(req_cart.segment_3_results.text).to eq "Red 100 (100) Red 200 (200)"
      end
    end

    shared_examples 'lookup values & account display for delhi_coa' do |user|
      scenario "for #{user}" do
        login_as user
        create_req_line
        req_cart.choose_acc.click
        # For Delhi COA
        select_coa delhi_coa.name if user == us_no_restriction_user.login
        wait_for_ajax
        select_coa delhi_coa.name if user == "austria_user_restrict_to_coa_only_AGs"
        wait_for_ajax
        # Segment 1: Should display only the blue 200 lookup value and blue 7000 lookup value
        req_cart.choose_an_account.segment[0].click
        expect(req_cart.segment_1_results.text).to eq "Blue 200 (200) Blue 7000 (7000)"
        req_cart.choose_segment[0].search_result[1].click
        # Segment 2: Should display only the green 1981 lookup value and green 2021 lookup value
        req_cart.choose_an_account.segment[1].click
        expect(req_cart.segment_2_results.text).to eq "Green 1981 (1981) Green 2021 (2021)"
        req_cart.choose_segment[1].search_result[0].click
        # Segment 3: depends on seg1 it should display the child value of the lookup value selected in the 1st segment
        req_cart.choose_an_account.segment[2].click
        expect(req_cart.segment_3_results.text).to eq "Blue 7001 (7001)"
        req_cart.choose_an_account.segment[2].click
        # Segment 4: depends on seg2 it should display the child value of the lookup value selected in the 2nd segment
        req_cart.choose_an_account.segment[3].click
        expect(req_cart.segment_4_results.text).to eq "Green 1982 (1982)"
        req_cart.choose_an_account.segment[3].click
        # Segment 5: should display only the red lookup values(Red 100/Red 200/Red 500/Red 600)
        req_cart.choose_an_account.segment[4].click
        expect(req_cart.segment_5_results.text).to eq "Red 100 (100) Red 200 (200) Red 500 (500) Red 600 (600)"
      end
    end

    shared_examples 'AG validation for delhi_coa' do |user|
      scenario "AG validation for delhi coa with #{user}" do
        login_as user
        create_req_line
        req_cart.choose_acc.click
        req_cart.choose_an_account.segment[0].click
        req_cart.select_lookupvalue_on_segment "Blue 7000 (7000)"
        req_cart.choose_an_account.segment[1].click
        req_cart.select_lookupvalue_on_segment "Green 2021 (2021)"
        req_cart.choose_an_account.segment[2].click
        req_cart.select_lookupvalue_on_segment "Blue 7001 (7001)"
        req_cart.choose_an_account.segment[3].click
        req_cart.select_lookupvalue_on_segment "Green 2022 (2022)"
        req_cart.choose_an_account.segment[4].click
        req_cart.select_lookupvalue_on_segment("Red 100 (100)")
        req_cart.choose_an_account.choose.click
        expect(req_cart.choose_an_account.errors[1]).to have_content(I18n.t('accounts_controller.your_security_settings_prevent_using_this_account'))
        req_cart.choose_an_account.segment[1].click
        req_cart.select_lookupvalue_on_segment "Green 1981 (1981)"
        req_cart.choose_an_account.choose.click
        expect(req_cart.req_cart_line.account_code).to have_content "7000-1981-7001--100"
      end
    end

    shared_examples 'lookup values & account display for qatar_coa' do |user|
      scenario "for #{user}" do
        login_as user
        create_req_line
        req_cart.choose_acc.click
        # For Qatar COA
        select_coa qatar_coa.name if user == us_no_restriction_user.login
        wait_for_ajax
        select_coa qatar_coa.name if user == "austria_user_restrict_to_coa_only_AGs"
        wait_for_ajax
        # The third segment should be locked by default and the value is came from the commodity.
        expect(req_cart.choose_an_account).to have_locked_segments
        expect(req_cart.locked_segment_3).to have_content "Some segments have been locked by your administrator. *3Seg 3 Red 200 (200)"
        # Segment 1: Should display only the red lookup values(Red 100/Red 200/Red 700/Red 800)
        req_cart.choose_an_account.segment[0].click
        expect(req_cart.segment_1_results.text).to eq "Red 100 (100) Red 200 (200) Red 700 (700) Red 800 (800)"
        req_cart.choose_an_account.segment[0].click
        # Segment 2: Should display only the red lookup values(Red 100/Red 200/Red 700/Red 800)
        req_cart.choose_an_account.segment[1].click
        expect(req_cart.segment_2_results.text).to eq "Red 100 (100) Red 200 (200) Red 700 (700) Red 800 (800)"
        req_cart.choose_an_account.segment[1].click
        # Segment 4: Should display only the red lookup values(Red 100/Red 200/Red 700/Red 800)
        req_cart.choose_an_account.segment[2].click
        expect(req_cart.segment_4_results.text).to eq "Red 100 (100) Red 200 (200) Red 700 (700) Red 800 (800)"
        req_cart.choose_an_account.segment[2].click
        # Segment 5: Should display only the red lookup values(Red 100/Red 200/Red 700/Red 800)
        req_cart.choose_an_account.segment[3].click
        expect(req_cart.segment_5_results.text).to eq "Red 100 (100) Red 200 (200) Red 700 (700) Red 800 (800)"
        req_cart.choose_an_account.segment[3].click
      end
    end

    shared_examples 'AG validation for qatar_coa' do |user|
      scenario "AG validation for qatar coa with #{user}" do
        login_as user
        create_req_line
        req_cart.choose_acc.click
        req_cart.choose_an_account.segment[0].click
        ...
        req_cart.choose_segment[1].search_result[1].click
        req_cart.choose_an_account.segment[2].click
        req_cart.choose_segment[2].search_result.last.click
        req_cart.choose_an_account.segment[3].click
        req_cart.choose_segment[3].search_result.last.click
        req_cart.choose_an_account.choose.click
        expect(req_cart.choose_an_account.errors[0]).to have_content(I18n.t('accounts_controller.your_security_settings_prevent_using_this_account'))
        req_cart.choose_an_account.segment[2].click
        req_cart.choose_segment[2].search_result[1].click
        req_cart.choose_an_account.choose.click
        expect(req_cart.account_show_read_only.text).to eq "200-200-200-200-800"
      end
    end

    shared_examples 'lookup values & account display for doha_coa' do |user|
      scenario "for #{user}" do
        login_as user
        create_req_line
        req_cart.choose_acc.click
        # For Doha COA
        select_coa doha_coa.name if user == us_no_restriction_user.login
        wait_for_ajax
        select_coa doha_coa.name if user == "austria_user_restrict_to_coa_only_AGs"
        wait_for_ajax
        # Segment 1: Should display only the blue 200 lookup value and blue 8000 lookup value
        req_cart.choose_an_account.segment[0].click
        expect(req_cart.segment_1_results.text).to eq "Blue 200 (200) Blue 8000 (8000)"
        req_cart.choose_segment[0].search_result[0].click
        # Segment 2: Should display only the green 1981 lookup value and green 1945 lookup value
        req_cart.choose_an_account.segment[1].click
        expect(req_cart.segment_2_results.text).to eq "Green 1945 (1945) Green 1981 (1981)"
        req_cart.choose_segment[1].search_result[0].click
        # Segment 3: depends on seg1 it should display the child value of the lookup value selected in the 1st segment
        req_cart.choose_an_account.segment[2].click
        expect(req_cart.segment_3_results.text).to eq "Blue 201 (201) Blue 202 (202)"
        req_cart.choose_an_account.segment[2].click
        # Segment 4: depends on seg2 it should display the child value of the lookup value selected in the 2nd segment
        req_cart.choose_an_account.segment[3].click
        expect(req_cart.segment_4_results.text).to eq "Green 1946 (1946)"
        req_cart.choose_an_account.segment[3].click
        # Segment 5: should display only the red lookup values(Red 100/Red 200/Red 500/Red 600)
        req_cart.choose_an_account.segment[4].click
        expect(req_cart.segment_5_results.text).to eq "Red 100 (100) Red 1000 (1000) Red 200 (200) Red 900 (900)"
        req_cart.choose_an_account.segment[4].click
      end
    end

    shared_examples 'AG validation for doha_coa' do |user|
      scenario "AG validation for doha_coa with #{user}" do
        login_as user
        create_req_line
        req_cart.choose_acc.click
        req_cart.choose_an_account.segment[0].click
        req_cart.select_lookupvalue_on_segment "Blue 200 (200)"
        req_cart.choose_an_account.segment[1].click
        req_cart.select_lookupvalue_on_segment "Green 1945 (1945)"
        req_cart.choose_an_account.segment[2].click
        req_cart.select_lookupvalue_on_segment "Blue 201 (201)"
        req_cart.choose_an_account.segment[3].click
        req_cart.select_lookupvalue_on_segment "Green 1946 (1946)"
        req_cart.choose_an_account.segment[4].click
        req_cart.select_lookupvalue_on_segment("Red 100 (100)")
        req_cart.choose_an_account.choose.click
        expect(req_cart.choose_an_account.errors[0]).to have_content(I18n.t('accounts_controller.your_security_settings_prevent_using_this_account'))
        req_cart.choose_an_account.segment[4].click
        req_cart.select_lookupvalue_on_segment("Red 200 (200)")
        req_cart.choose_an_account.choose.click
        expect(req_cart.req_cart_line.account_code).to have_content "200-1945-201-1946-200"
      end
    end

    shared_examples "account display for fiji_default_chennai_coa_user" do |user|
      scenario "account display for different users selecting different COA for #{user}" do
        login_as user
        create_req_line
        ##### account display #####
        req_cart.account_code.click
        req_cart.set_account.set "200"
        accounts = req_cart.autocomplete[1].text
        displayed_accounts = accounts.split(',')
        expect(displayed_accounts).to match_array (["Red 100-Red 200-Red 200: 100-200-200 Red 100-Red 200: 100-200 Red 100-Red 400-Red 200: 100-400-200 Red 200-Red 300-Red 100: 200-300-100 Red 100-Red 200-Red 400: 100-200-400 Red 100-Red 200-Red 100: 100-200-100 Red 100-Red 100-Red 200: 100-100-200 Red 100-Red 300-Red 200: 100-300-200"])
      end
    end

    shared_examples "account display for colombia_default_delhi_coa_user" do |user|
      scenario "account display for different users selecting different COA for #{user}" do
        login_as user
        create_req_line
        ##### account display #####
        ...
        accounts = req_cart.autocomplete[1].text
        displayed_accounts = accounts.split(',')
        expect(displayed_accounts).to match_array (["Blue 200--Blue 201--Red 200: 200--201--200 Blue 200-Green 2021-Blue 201--Red 200: 200-2021-201--200 Blue 200-Green 2021-Blue 201-Green 2022-Red 200: 200-2021-201-2022-200 Blue 200-Green 1981-Blue 202-Green 1982-Red 200: 200-1981-202-1982-200 Blue 200-Green 1981-Blue 201-Green 1982-Red 200: 200-1981-201-1982-200 Blue 200-Green 1981-Blue 201-Green 1982-Red 500: 200-1981-201-1982-500 Blue 200-Green 1981-Blue 201-Green 1982-Red 100: 200-1981-201-1982-100 Blue 7000-Green 1981-Blue 7001-Green 1982-Red 200: 7000-1981-7001-1982-200"])
      end
    end

    shared_examples "account display for india_default_qatar_coa_user" do |user|
      scenario "account display for different users selecting different COA for #{user}" do
        login_as user
        create_req_line
        ##### account display #####
        req_cart.account_show_read_only.click
        # Since this user is restricted to qatar_coa and it has locking concept we display the choose account popup instead of show account account_code_autocomplete_list
        expect(req_cart).to have_choose_an_account
        expect(req_cart.choose_an_account).to have_locked_segments
        expect(req_cart.locked_segment_3).to have_content "Some segments have been locked by your administrator. *3Seg 3 Red 200 (200)"
      end
    end

    shared_examples "account display for japan_default_doha_coa_user" do |user|
      scenario "account display for different users selecting different COA for #{user}" do
        login_as user
        create_req_line
        ##### account display #####
        ...
        accounts = req_cart.autocomplete[1].text
        displayed_accounts = accounts.split(',')
        expect(displayed_accounts).to match_array (["Blue 8000-Green 1981-Blue 8001-Green 1982-Red 200: 8000-1981-8001-1982-200"])
      end
    end

    shared_examples "account display for china_user_restrict_to_chennai_coa_ag_by_seg" do |user|
      scenario "account display for different users selecting different COA for #{user}" do
        login_as user
        create_req_line
        ##### account display #####
        ...
        expect(displayed_accounts).to match_array (["Red 200-Red 300-Red 100: 200-300-100 Red 100-Red 300-Red 200: 100-300-200"])
      end
    end

    shared_examples "account display for egypt_user_restrict_to_delhi_coa_ag_by_seg" do |user|
      scenario "account display for different users selecting different COA for #{user}" do
        login_as user
        create_req_line
        ##### account display #####
        ...
        accounts = req_cart.autocomplete[1].text
        displayed_accounts = accounts.split(',')
        # accounts which has 200 as segemnt valua and satisfies the delhi_coa_ag_by_seg
        expect(displayed_accounts).to match_array (["Blue 200-Green 1981-Blue 202-Green 1982-Red 200: 200-1981-202-1982-200 Blue 200-Green 1981-Blue 201-Green 1982-Red 200: 200-1981-201-1982-200 Blue 200-Green 1981-Blue 201-Green 1982-Red 500: 200-1981-201-1982-500 Blue 200-Green 1981-Blue 201-Green 1982-Red 100: 200-1981-201-1982-100 Blue 7000-Green 1981-Blue 7001-Green 1982-Red 200: 7000-1981-7001-1982-200"])
      end
    end

    shared_examples "account display for paris_user_restrict_to_qatar_coa_ag_by_seg" do |user|
      scenario "account display for different users selecting different COA for #{user}" do
        login_as user
        create_req_line
        ##### account display #####
        ...
        accounts = req_cart.autocomplete[1].text
        displayed_accounts = accounts.split(',')
        # accounts which has 200 as segemnt valua and satisfies the qatar_coa_ag_by_seg
        expect(displayed_accounts).to match_array (["Red 100-Red 200-Red 700-Red 200-Red 200: 100-200-700-200-200"])
      end
    end

    shared_examples "account display for italy_user_restrict_to_doha_coa_ag_by_seg" do |user|
      scenario "account display for different users selecting different COA for #{user}" do
        login_as user
        create_req_line
        ##### account display #####
        ...
        accounts = req_cart.autocomplete[1].text
        displayed_accounts = accounts.split(',')
        # accounts which has 200 as segemnt valua and satisfies the doha_coa_ag_by_seg
        expect(displayed_accounts).to match_array (["Blue 8000-Green 1981-Blue 8001-Green 1982-Red 200: 8000-1981-8001-1982-200"])
      end
    end

    shared_examples "account display for us_no_restriction_user" do |user|
      scenario "account display for different users selecting different COA for #{user}" do
        login_as user
        create_req_line
        #### account display #####
        ...
        accounts = req_cart.autocomplete[1].text
        displayed_accounts = accounts.split(',')
        expect(displayed_accounts).to match_array (["Red 100-Red 200-Red 700-Red 200-Red 200: 100-200-700-200-200 (Qatar_COA) Red 100-Red 200-Red 200: 100-200-200 (Chennai_COA) Blue 200--Blue 201--Red 200: 200--201--200 (Delhi_COA) Red 100-Red 200-Red 200: 100-200-200 (Pune_COA) Blue 200-Green 2021-Blue 201--Red 200: 200-2021-201--200 (Delhi_COA) Blue 200-Green 2021-Blue 201-Green 2022-Red 200: 200-2021-201-2022-200 (Delhi_COA) Red 700-Red 200-Red 200-Red 800-Red 100: 700-200-200-800-100 (Qatar_COA) Blue 200-Green 1981-Blue 202-Green 1982-Red 200: 200-1981-202-1982-200 (Delhi_COA) Blue 200-Green 1981-Blue 201-Green 1982-Red 200: 200-1981-201-1982-200 (Delhi_COA) Red 100-Red 200-Red 700-Red 800-Red 200: 100-200-700-800-200 (Qatar_COA)"])
      end
    end

    shared_examples "lookup and account display on split billing" do |user|
      scenario "lookup and account display on split billing #{user}" do
        login_as user
        create_req_line
        req_cart.split_billing_icon.click
        req_cart.split_billing_popup.wait_for_split_account_lines
        req_cart.split_billing_popup.split_account_lines[1].pick_account.click
        req_cart.wait_for_choose_an_account
        select_coa chennai_coa.name
        wait_for_ajax
        # For Chennai COA By default the third segment should be defaulted from commodity
        expect(req_cart.choose_an_account.segment[2]).to have_content "Red 200 (200)"
        # The frist 4 lookup values from the Red lookup should be displayed
    ...
    context 'feature off' do
      ...
      it_behaves_like "lookup values & account display for chennai_coa", "us_no_restriction_user"
      it_behaves_like "lookup values & account display for pune_coa", "us_no_restriction_user"
      it_behaves_like "lookup values & account display for delhi_coa", "us_no_restriction_user"
      it_behaves_like "lookup values & account display for qatar_coa", "us_no_restriction_user"
      it_behaves_like "lookup values & account display for doha_coa", "us_no_restriction_user"

      # user restricted to default COA
      it_behaves_like "lookup values & account display for chennai_coa", "fiji_default_chennai_coa_user"
      it_behaves_like "lookup values & account display for delhi_coa", "colombia_default_delhi_coa_user"
      it_behaves_like "lookup values & account display for qatar_coa", "india_default_qatar_coa_user"
      it_behaves_like "lookup values & account display for doha_coa", "japan_default_doha_coa_user"

      # user restricted to specific account group
      it_behaves_like "lookup values & account display for chennai_coa", "china_user_restrict_to_chennai_coa_ag_by_seg"
      it_behaves_like "lookup values & account display for delhi_coa", "egypt_user_restrict_to_delhi_coa_ag_by_seg"
      it_behaves_like "lookup values & account display for qatar_coa", "paris_user_restrict_to_qatar_coa_ag_by_seg"
      it_behaves_like "lookup values & account display for doha_coa", "italy_user_restrict_to_doha_coa_ag_by_seg"

      it_behaves_like 'AG validation for chennai_coa', 'china_user_restrict_to_chennai_coa_ag_by_seg'
      it_behaves_like 'AG validation for delhi_coa', "egypt_user_restrict_to_delhi_coa_ag_by_seg"
      it_behaves_like 'AG validation for qatar_coa', "paris_user_restrict_to_qatar_coa_ag_by_seg"
      it_behaves_like 'AG validation for doha_coa', "italy_user_restrict_to_doha_coa_ag_by_seg"

      # accounts display for different users
      it_behaves_like "account display for us_no_restriction_user", "us_no_restriction_user"
      it_behaves_like "account display for fiji_default_chennai_coa_user", "fiji_default_chennai_coa_user"
      it_behaves_like "account display for colombia_default_delhi_coa_user", "colombia_default_delhi_coa_user"
      it_behaves_like "account display for india_default_qatar_coa_user", "india_default_qatar_coa_user"
      it_behaves_like "account display for japan_default_doha_coa_user", "japan_default_doha_coa_user"
      it_behaves_like "account display for china_user_restrict_to_chennai_coa_ag_by_seg", "china_user_restrict_to_chennai_coa_ag_by_seg"
      it_behaves_like "account display for italy_user_restrict_to_doha_coa_ag_by_seg", "italy_user_restrict_to_doha_coa_ag_by_seg"
      it_behaves_like "account display for egypt_user_restrict_to_delhi_coa_ag_by_seg", "egypt_user_restrict_to_delhi_coa_ag_by_seg"
      it_behaves_like "account display for paris_user_restrict_to_qatar_coa_ag_by_seg", "paris_user_restrict_to_qatar_coa_ag_by_seg"
    end
    ...
    context 'feature on' do
      ...
      given!(:austria_user_restrict_to_coa_only_AGs) { FactoryGirl.create(:user, :with_roles, roles: "User, Buyer, Accounts Payable, Expense User , Expense Processor", login: "austria_user_restrict_to_coa_only_AGs", business_group_security_type: 0, account_security_type: 2, account_groups: [chennai_coa_ag_by_coa, delhi_coa_ag_by_coa, qatar_coa_ag_by_coa, doha_coa_ag_by_coa]) }
      ...
      # user restricted to COA only account group
      it_behaves_like "lookup values & account display for chennai_coa", "austria_user_restrict_to_coa_only_AGs"
      it_behaves_like "lookup values & account display for delhi_coa", "austria_user_restrict_to_coa_only_AGs"
      it_behaves_like "lookup values & account display for qatar_coa", "austria_user_restrict_to_coa_only_AGs"
      it_behaves_like "lookup values & account display for doha_coa", "austria_user_restrict_to_coa_only_AGs"
      ...
      it_behaves_like "lookup and account display on split billing", "austria_user_restrict_to_coa_only_AGs"
  ...
  def select_coa(coa_name)
    req_cart.choose_an_account.account_type.select coa_name
  end

  def create_req_line
    req_cart.load
    req_cart.add_line_link.click
    req_cart.req_cart_line.req_line_description.set item.name
    click_autocomplete_row
    req_cart.save_cart.click
    sleep 1
  end
end
